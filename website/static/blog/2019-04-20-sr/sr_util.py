...
def run_sr(emis, model, output_variables, emis_units="tons/year"):
    ...
    if _inmap_exe == None:
        ...
        if ost == "Windows":
            ...
            _download("https://github.com/yuzhou-wang/inmap/releases/download/v%s/inmap-v%s-windows-%s.exe"%(version, version, arch), _inmap_exe)
        elif ost == "Darwin":
            _inmap_exe = os.path.join(_tmpdir.name, "inmap_%s"%version)
            _download("https://github.com/yuzhou-wang/inmap/releases/download/v%s/inmap-v%s-darwin-%s"%(version, version, arch), _inmap_exe)
        elif ost == "Linux":
            _inmap_exe = os.path.join(_tmpdir.name, "inmap_%s"%version)
            _download("https://github.com/yuzhou-wang/inmap/releases/download/v%s/inmap-v%s-linux-%s"%(version, version, arch), _inmap_exe)
        else:
